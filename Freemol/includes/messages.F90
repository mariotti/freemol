...
!H-----------------------------------------------------------------------------
!H $Id: messages.F90,v 1.1.1.1 2009/01/12 16:56:08 mariotti Exp $
!H-----------------------------------------------------------------------------
!H 
!H 
!
...
!H
!H     messages_lastcode
!H                   It contains the last colled code in the messages module.
...
!H      
!H      call message(<mescode>,truemessage)
!H      
...
!H
!H      Actually the message_init routine is checked before any call
!H      just to be safe if you use it uncoditionally. The actual defaults
!H      are a call to messages itself of the type:
!H      call messages_initw(6,6,0,6,1,messages_debug)
!H      which sets as default values for fortran units:
!H      6 for messages
!H      6 for output
!H      0 for errors
!H      6 for log
!H      1 as mode (not yet used)
!H      the debug value to the default messages_debug
!H
!H      Please NOTE that it is the only modules that uses STD fortran subroutines
!H      because you shouldnt expect any return code. The idea is that you wont have
!H      any unit where eventually print out the error code: i.e. if messages fails
!H      you wont have any possibility to get back your own error!
!H      It doesnt use any other module but vartypes so it should be quite safe!
!H      
...
!H      in the form:
!H      MESXXXX: <message>
!H      I suggest a 70 max length for the message in order to have 80 column
!H      result string.
...
!H      The module is not meant to produce output! But to produce messages
!H      in the output and other eventual file units. Thats why it print out
!H      a # character, so you can strip/grep it out!
!H      
...
!
  subroutine message_valuei(icode,cmes,ival,imode)
    ...
    val_frmt=trim(val_frmt)
    i=len_trim(val_frmt)
    if(i.gt.(FLCHARS-2)) then
       stop 'Internal message routine error'
    endif
    val_frmt='('//val_frmt(1:i)//')'
    write(val_data,val_frmt(1:i+2)) ival
...
!
  subroutine message_valuer(icode,cmes,rval,imode)
    ...
    val_frmt = '('//val_frmt(1:i)//')'
    i = len_trim(val_frmt)
    write(val_data,val_frmt(1:i)) rval
    newmessage = cmes(1:len_trim(cmes))//' '//val_data
    !
    ! than we call the normal message routine
    if (present(imode)) then
