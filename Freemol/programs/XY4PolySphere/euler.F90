...
!this subroutine calculates the Euler angles for the rotation of any    
!nuclear frame given by the nuclear positions Y of the NATOM nuclei     
!to the Eckart reference frame (NATOM = N/3) and performs the rotation. 
!                                                                       
      IMPLICIT DOUBLE PRECISION(A-H,O-Z) 
        ...
        ZABS(X)=ABS(X) 
        IRC=0 
!test Schwerpunktsystem und Haupttraegheitsachsen                       
        NTEST=N/3 
    ...
    3    SUM3=SUM3+U(J)*XE(3*J)*XE(3*J-2) 
        ...
        IF (SUM.GT.TOL) THEN 
         ...
         RETURN 
        ENDIF 
!calculate cross products of XE with Y                                  
        DO 4 K=1,3 
        DO 4 I=1,3 
         SUM=0.0 
         DO 5 J=0,(NATOM-1) 
    ...
    4    CP(I,K)=SUM 
!        print '(3(x,g16.9))', (cp(1,k),k=1,3)                          
!        print '(3(x,g16.9))', (cp(2,k),k=1,3)                          
!        print '(3(x,g16.9))', (cp(3,k),k=1,3)                          
!inital guesses for THETA, PHI and PSI                                  
!     READ(*,*) (ANGLES(I),I=1,3)                                       
!      READ(*,*) NTRIAL                                                 
        ANGLES(1) = THETA 
        ANGLES(2) = PHI 
...
!solve for roots of Eckart equations with Newton-Raphson                
!       NTRIAL=MTRIAL                                                   
        NROOT=3 
        TOLX=TOL 
        TOLF=TOL 
...
!check results (test 1)                                                 
        ...
        WRITE(*,*) (F(I),I=1,3) 
!test 2: rotate coordinates and check eckart conditions                 
   11 CONTINUE 
      DO 300 J=0,(NATOM-1) 
       XX(1)=Y(J*3+1) 
       XX(2)=Y(J*3+2) 
       ...
       Y(J*3+3)=YY(3) 
  300 END DO 
!calculate cross products of XE with rotated Y (test 2)                 
        DO 304 K=1,3 
        DO 304 I=1,3 
         SUM=0.0 
         DO 305 J=0,(NATOM-1) 
...
!this subroutine calculates the Eckart conditions suitable for use      
...
!                                                                       
      IMPLICIT DOUBLE PRECISION(A-H,O-Z) 
        ...
        CHI(3,2)= STHE*SPHI 
        CHI(3,3)= CTHE 
!        print '(3(x,g16.9))', (chi(1,k),k=1,3)                         
!        print '(3(x,g16.9))', (chi(2,k),k=1,3)                         
!        print '(3(x,g16.9))', (chi(3,k),k=1,3)                         
...
!this subroutine rotates the vector Y given in a molecule fixed frame   
!to  a vector X given in the Eckart frame.                              
!                                                                       
      IMPLICIT DOUBLE PRECISION(A-H,O-Z) 
...
!this subroutine rotates the vector Y given in the Eckart frame into    
!a vector X given in the moleule fixed frame                            
!                                                                       
      IMPLICIT DOUBLE PRECISION(A-H,O-Z) 
        DIMENSION CHI(3,3),X(3),Y(3) 
        ZCOS(R)=COS(R) 
