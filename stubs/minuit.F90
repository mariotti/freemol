...
module minuit
  ...
  !Minuit I/O units
  !----------------
  integer(FINT) :: minuit_filein !OLD:isysrd
  !
  ! Others
  !-------
  ...
  !
  integer(FINT), save :: icomnd, istrat, isyssa, isyswr, itaur
  ...
  integer(FINT), save, dimension(mni) :: nexofi !mni
...
!--       is = internal number of parameter being restored
    ...
    integer(FINT) :: k, ka, ik, ipsav, ir, is, lc, iq
...
!                    remove one row and one column from variance matrix
      ...
      do i= 1, nold
         do j= 1, i
            kold = kold + 1
            if (.not.(j.eq.iint.or.i.eq.iint)) then
               knew = knew + 1
...
!  alphabetical order of command names!
     ...
     integer(FINT) :: nsuper, ilist, iext, iint, ierr, krl, it
...
!           sure that word7(1)=0. if llist=0
     ...
     if (cword(1:7).ne.'SET PRI'.or.word7(1).ge.0.)  then
        if (isw(5).ge.0) then
           lnow = llist
           if (lnow.gt.4)  lnow=4
           write (isyswr,"(1H ,10(1H*)/' **',I5,' **',A,4G12.4)")&
...
!c
    ...
    !
    integer(FINT) :: npfn, nparp1, nparx, jl, kg, ns, nf, k, jh, j
...
!c
    ...
    character cbf1*22
    !
    integer(FINT) :: ncyc, npard, nparx, npar2, idrv, id, iext
...
!                         calculate external error if v exists
    !
    integer(FINT) :: l, ndex, k1, j, k, ierr, iin, ndiag
...
!c        if not, adds constant along diagonal to make positive.
    ...
    !
    integer(FINT) :: ndex, ndexd, j, ifault, ip
...
!
  subroutine mneig(a,ndima,n,mits,work,precis,ifault)
    ...
    !
    integer(FINT) :: i1, l, k, j, j1, n1, i0, m1, m
...
!c
    ...
    character cbf1*22
    !
    integer(FINT) :: nparx, ncyc, icyc, iint, iext
...
!c
    ...
    real(FREAL) :: bwidx, ymax, ymin, dyy, bwidy, any, ax, ay, bx, by
    !
...
!c
    ...
    real(FREAL) :: zlim, anext, fnext, dfda, maxlk, fdist, adist
...
1100 continue
    ...
    write (ISYSWR,1002) CMODE,minuit_filein,CFNAME
...
!c        retains the best function and parameter values found.
    !
    !
    integer(FINT) :: ncall, nccall, iparwd, ipar, iint, nxypt
...
!c
    ...
    !
    integer(FINT) :: nlines, iint, npar2, ncovar
    !
!
