...
!H----------------------------------------------------------------------
!H----------------------------------------------------------------------
!H $Id: linetools.F90,v 1.6 2002/02/25 05:32:35 fmariot Exp $
!H----------------------------------------------------------------------
!H
!
module linetools
...
!
    ...
    !
    if(lback) then
       i = index(line,lkey(1:ilen))
    else
       i = index(line,lkey(1:ilen),lback)
    end if
    ...
    !---------------------------------
    if((i+ilen-1).lt.len(line)) then
       if(line(i+ilen:i+ilen).ne.' ') then
          windexr = -1
       end if
    endif
    !
    ...
    if (windexr.eq.-1) then
       if ((i+ilen).le.len(line)) then
          i = windex(line(i+ilen:len(line)),lkey,lback)
       end if
    end if
    !
    windexr = i
